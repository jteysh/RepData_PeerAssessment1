...
```r
total_steps_pd <- activityDT[, c(lapply(.SD, sum, na.rm = FALSE)), .SDcols = c("steps"), by = .(date)] 
...
```r
# Using median of dataset to fill in missing values. 
...
Yes, mean (10,765) and median (10,765) with missing data were larger than mean (9,354.23) and median (10,395) with imputed missing data. 
